...
Example:
    ...
    curl -sSL https://raw.githubusercontent.com/idelchi/envprof/refs/heads/dev/install.sh | GITHUB_TOKEN=<token> sh -s -- -d ~/.local/bin

EOF
  curl ${ENVPROF_DISABLE_SSL:+-k} -sSL https://raw.githubusercontent.com/idelchi/scripts/refs/heads/dev/install.sh | INSTALLER_TOOL="envprof" sh -s -- -h
...
install() {
  ...
  curl ${DISABLE_SSL:+-k} -sSL https://raw.githubusercontent.com/idelchi/scripts/refs/heads/dev/install.sh | ENVPROF_GITHUB_TOKEN=${GITHUB_TOKEN} INSTALLER_TOOL="envprof" sh -s -- "$@"
}
