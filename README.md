<p align="center">
  <img alt="envsync logo" src="assets/envsync.png" height="150" />
...
[![GitHub release](https://img.shields.io/github/v/release/idelchi/envsync)](https://github.com/idelchi/envsync/releases)
...
```sh
curl -sSL https://raw.githubusercontent.com/idelchi/envsync/refs/heads/dev/install.sh | sh -s -- -d ~/.local/bin
```

## Usage

A single root-level flag `--file` can be used to specify the configuration file. If not provided, it defaults to `envsync.yaml`, `envsync.yml`, `envsync.toml`, or `envsync.json` in the current directory.

```sh
# add adds or updates a variable
envsync add dev DB_URL postgres://localhost:5432
...
dev:
  extends: [base] # or `extends: base` for single profile extension
  env:
    DB_NAME: devdb
    DEBUG: "true"
```
